...
const CustomerReviews = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prevIndex) => {
        // Loop back to the first set of reviews once the end is reached
        return prevIndex + 4 >= reviews.length ? 0 : prevIndex + 4;
      });
    }, 2000); // 2-second interval to change the review batch
  ...
  return (
    ...
    <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <h2 className="mb-12 text-2xl text-left text-black font-quando">
        CUSTOMER REVIEWS
      </h2>
      <div
        className="grid grid-cols-1 gap-8 overflow-hidden sm:grid-cols-2 lg:grid-cols-4"
        ref={reviewRef}
      >
        {/* Render 4 reviews at a time */}
        {reviews.slice(currentIndex, currentIndex + 4).map((review, index) => (
          ...
          >
            ...
            <div
              ...
              style={{ borderBottom: '3px solid #D1D5DB' }} // Custom gray color with 3px height
            ></div>
          </div>
        ))}
      </div>
                <a
            href=""
            className="block p-10 text-right text-black transition duration-300 font-poppins hover:text-blue-700"
            >
            Add Your Review
            </a>
